...
// };
   vector<int> ans;
    ...
    vector<int> spiralOrder(vector<vector<int>>& matrix) {
      ...
      int n=matrix.size();
      int m=matrix[0].size();
      int fr=0, lr=n, fc=0, lc=m;
      while(fr<lr && fc<lc){
          for(int j=fc;j<lc;j++) ans.push_back(matrix[fr][j]);
          fr++;
          if(fr>=lr || fc>=lc) break;
          for(int i=fr;i<lr;i++) ans.push_back(matrix[i][lc-1]);
          lc--;
          if(fr>=lr || fc>=lc) break;
          for(int j=lc-1;j>=fc;j--) ans.push_back(matrix[lr-1][j]);
          lr--;
          if(fr>=lr || fc>=lc) break;
          for(int i=lr-1;i>=fr;i--) ans.push_back(matrix[i][fc]);
          fc++;
