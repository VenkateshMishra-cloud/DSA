...
vector<int> dp(n,1e9);
     for(int k=1;k<=n;k++){
         for(int i=0;i<n;i++){
             ...
             if(k==1){
                 ...
                 if(str[i]==str[i+k-1]) ispal[i][i+k-1]=1;
                 else ispal[i][i+k-1]=0;
             }
             else{
                 if(str[i]==str[i+k-1] && ispal[i+1][i+k-2]==1) ispal[i][i+k-1]=1;
                 else ispal[i][i+k-1]=0;
     ...
     for(int i=0;i<n;i++){
         for(int j=i;j>=0;j--){
            if(j==0){
                if(ispal[j][i]==1) dp[i]=1;
            }
            else{
                if(ispal[j][i]==1 && dp[j-1]!=0) dp[i]=min(1+dp[j-1],dp[i]);
            }
         }
     }
     cout<<dp[n-1]-1<<endl;
