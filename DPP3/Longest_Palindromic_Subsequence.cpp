  ...
  bool operator()(pair<int,int> &v1,pair<int,int> &v2)
   {
    ...
    int helper(string& a, string& b, vector<vector<int>>& dp, int l, int r, int n){
        if(l==n || r==n) return 0;
        if(dp[l][r]!=-1) return dp[l][r];
        if(a[l]==b[r]){
            return dp[l][r]=1+helper(a,b,dp,l+1,r+1,n);
        }
        int x1=helper(a,b,dp,l+1,r,n);
        int x2=helper(a,b,dp,l,r+1,n);
        return dp[l][r]=max(x1,x2);
    ...
    int longestPalinSubseq(string a) {
        ...
        int ans=0;
        int x= helper(a,b,dp,0,0,n);
        for(int i=0;i<n;i++){
            for(int j=0;j<n;j++) ans=max(dp[i][j],ans);
